...
class Pulse(object):
    ...
    def calculate_waveform(self, t0, t):
        ...
        if self.pulse_type in (PulseType.XY, ):
            # Apply phase and SSB
            phase = self.phase
        ...
        if self.pulse_type in (PulseType.READOUT, ):
            ...
            omega = 2 * np.pi * self.frequency
            # apply SSBM transform
            data_i = self.iq_ratio * (y.real * np.cos(omega * (t - t0) - phase) +
                                      - y.imag * np.cos(omega * (t - t0) - phase +
                                                        np.pi / 2))
            data_q = (y.real * np.sin(omega * (t - t0) - phase + self.iq_skew) +
                      -y.imag * np.sin(omega * (t - t0) - phase + self.iq_skew +
                                       np.pi / 2))
            # subtracted offset time t0 for readout pulses.
            y = data_i + 1j * data_q
