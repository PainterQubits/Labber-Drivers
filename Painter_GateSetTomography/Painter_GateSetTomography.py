...
import gate_set_tomography as gst
...
class Driver(InstrumentDriver.InstrumentWorker):
    ...
    def performOpen(self, options={}):
        ...
        # Initialize empty experiment set
        self.experiments = []
    ...
    def performSetValue(self, quant, value, sweepRate=0.0, options={}):
        ...
        if quant.name == 'Percent of Sequence':
            if not 0 <= value <= 100:
                value = 100
        if quant.name == 'Experiment Index':
            if value < -4:
                value = 0
        if 'Sigmas' in quant.name:
            if value <= 0:
                value = 1
            value = np.ceil(value)
        if quant.name == 'Path to Experiment Config':
            self.experiments = gst.parse_gst_file(value)
#          try:
#              self.experiments = gst.parse_gst_file(quant.value)
#          except Exception e:
#              raise(e)
#              self.experiments = []
    ...
    def performGetValue(self, quant, options={}):
        ...
        if 'Waveform' in quant.name:
            ...
            if quant.name == 'Waveform - X Signal':
                ...
                signal = np.concatenate((self.readout_trigger_heralding,
                                         self.readout_trigger))
                trace = quant.getTraceDict(signal, t0=0.0, dt=self.dt)
            return trace
    ...
    def update_clifford_generators(self):
        ...
        pi_pulse_post_delay = self.getValue('Pi Pulse Delay After Pulse') \
                                * sampling_rate
        ...
        pi_half_pulse_post_delay = \
                            self.getValue('Pi-Half Pulse Delay After Pulse') \
                            * sampling_rate
        ...
        pi_half_params = {
            ...
            'delay': pi_half_pulse_post_delay,
        }

        self.clifford_generators = gst.build_generator_signals(sampling_rate,
                                       pi_params,
                                       pi_half_params,
                                       const_detuning=True)
    ...
    def update_clifford_sequence(self):
        ...
        experiment_index = int(self.getValue('Experiment Index'))

        if experiment_index >= len(self.experiments):
            experiment_index = -1

        # Use special sees -4 to -1 for single pulses for testing / calibration
        if experiment_index >= 0:
            # Update sequence
            self.clifford_sequence = self.experiments[experiment_index]
        else:
    ...
    def update_xyz_sequences(self):
        ...
        # Update sequences
        x, y, z, times = gst.build_full_pulse_sequence(self.clifford_sequence,
                             self.clifford_generators,
                             sampling_rate)
